...
The first pass keeps the shape deliberately small:

1. prove Server1/OpenStack capacity and egress;
2. ensure a cattle Debian AI VM exists on `lab-internal`;
3. install Ollama directly on Server1 for inference;
4. run OpenWebUI in a container on Server1;
5. record the guarded Ollama/OpenWebUI fragments for the follow-up video.
...
- Ollama runs bare metal on Server1 because that keeps inference close to the
  proven CPU/RAM/disk capacity and avoids inventing tenant-network jump routing.
- OpenWebUI runs in a container and talks to Ollama on Server1 loopback.
