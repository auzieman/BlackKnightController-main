...
BUILTIN_ACTIONS = [
    {
        ...
        "label": "Probe host",
        "kind": "ssh",
        "summary": "Collect host identity, OS, kernel, disks, mounts, service, runtime, repo, and program-version facts over SSH.",
        "target_kinds": ["host", "vm", "kubernetes-node"],
        "credential_scope": "ssh:admin",
        "risk": "low",
        "inputs": {},
        "validations": ["hostname returned", "probe completes without mutating target"],
        "produces": [
            "fact:ssh_reachable",
            "fact:hostname",
            "fact:kernel",
            ...
            "fact:filesystems",
            "fact:mounts",
            "fact:running_services",
            "fact:program_versions",
            "fact:repo_states",
        ],
        "status": "implemented",
    },
    {
        "id": "ssh.facter.ensure",
        "label": "Ensure Facter facts",
        "kind": "ssh",
        "summary": "Pragmatically install or validate facter on a target, then collect normalized facter output for inventory enrichment.",
        "target_kinds": ["host", "vm", "kubernetes-node"],
        "credential_scope": "ssh:admin",
        "risk": "medium",
        "inputs": {"install_if_missing": "bool", "package_name": "string"},
        "validations": ["facter --version exits zero", "facter emits structured facts"],
        "produces": ["fact:facter_version", "fact:structured_host_facts"],
        "status": "planned",
    },
    {
    ...
    {
        "id": "ssh.lvm.grow_root",
        "label": "Grow root logical volume",
        "kind": "ssh",
        "summary": "Preflight and idempotently grow an LVM-backed root filesystem while retaining VG reserve.",
        "target_kinds": ["host", "vm", "kubernetes-node", "cluster"],
        "credential_scope": "ssh:admin",
        "risk": "high",
        "inputs": {"hosts": "list", "target_size_gib": "integer"},
        "validations": ["root is LVM-backed", "VG has sufficient free extents", "filesystem reports target size"],
        "produces": ["fact:root_filesystem_size", "fact:volume_group_reserve"],
        "status": "implemented",
    },
    {
        "id": "k3s.nodes.ready",
